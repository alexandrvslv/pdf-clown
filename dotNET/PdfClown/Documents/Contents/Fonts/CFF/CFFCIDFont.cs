...
namespace PdfClown.Documents.Contents.Fonts.CCF
{
	...
	public class CFFCIDFont : CFFFont
	{
		...
		private readonly Dictionary<int, CIDKeyedType2CharString> charStringCache =
		...
		private int GetDefaultWidthX(int gid)
		{
			...
			Dictionary<string, object> privDict = this.privateDictionaries.get(fdArrayIndex);
...
private int selectorToCID(string selector)
{
	...
	return int.parseInt(selector.substring(1));
}
